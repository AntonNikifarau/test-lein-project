...
(def student-tbl (csv/parse-csv (slurp "resources/csvdb/student.csv")))
(def subject-tbl (csv/parse-csv (slurp "resources/csvdb/subject.csv")))
(def student-subject-tbl (csv/parse-csv (slurp "resources/csvdb/student_subject.csv")))
...
(defn data-table [tbl]
  (let [keys (table-keys tbl)] (map #(data-record keys %) (next tbl))))
...
(defn str-field-to-int [field rec]
  (->> rec (field) (Integer/parseInt) (assoc rec field)))
...
(defn where* [data condition-func]
  (if condition-func (filter condition-func data) data))
...
(defn order-by* [data column]
  (if column (sort-by column data) data))
...
(defn join* [data1 column1 data2 column2]
  ...
  ((comp vec flatten) (map (fn [row] (map #(merge % row) (filter #(== (column1 row) (column2 %)) data2))) data1)))
